...
public final class CartaPagarSuerte extends CartaSuerte{ //Las clases hoja de una jerarquía deberían ser finales

    private double cantidad; //Positivo => Pagar , Negativo => Cobrar
    private boolean banca; //True => Banca, False => Todos los jugadores

    public CartaPagarSuerte(double cantidad, boolean banca, String descripcion, Valor valor){
        ...
        if(this.cantidad == 0) {
            ...
            for(Propiedades comp : jugador.getPropiedades()){
                if(comp instanceof Solar){
                    totalImpuestos += ((Solar) comp).getConstrucciones("Casa").size() * 40;
                    totalImpuestos += ((Solar) comp).getConstrucciones("Hotel").size() * 115;
                    totalImpuestos += ((Solar) comp).getConstrucciones("Piscina").size() * 200;
                    totalImpuestos += ((Solar) comp).getConstrucciones("Pista").size() * 75;
                }
            }
